...
'移出回收站
	...
	if G("act")="huanyuan" then
			if G("id")<>"" then 
				id=G("id")
				result=db.UpdateRecord("kl_articles","id="&id,array("recycling:0"))
					if result=0 then
		...
		if G("batchid")<>"" then
			...
			for i=0 to ubound(arr)
				result=db.UpdateRecord("kl_articles","id="&arr(i),array("recycling:0"))
			next
				if result=0 then
					AlertMsg(CAOZUO_FAIL_STR)
				end if
...
'彻底删除
	if G("act")="del" then
			if G("id")<>"" then 
				id=G("id")
				result=db.DeleteRecord("kl_articles","id",id)
					if result=0 then
						AlertMsg(CAOZUO_FAIL_STR)
					end if
			end if
		...
		if G("batchid")<>"" then
			...
			result=""
			for i=0 to ubound(arr)
				result=db.DeleteRecord("kl_articles","id",arr(i))
			next
				if result=0 then
					AlertMsg(CAOZUO_FAIL_STR)
				end if
...
'sql语句
mypage.getsql="SELECT a.id,a.arctitle,a.arcfbdate,a.arcflag,a.arcuddate,b.cat_name,c.type_name,a.arccontent,a.arcpic,a.recycling  from (kl_articles as a inner join  kl_cats as b on a.cat_id=b.cat_id) inner join kl_content_types as c on a.type_id=c.type_id  where recycling=1 order by arcfbdate desc"
'设置每一页的记录条数据为5条
mypage.pagesize=15
'返回Recordset
set rs=mypage.getrs()
